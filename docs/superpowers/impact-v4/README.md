...
| J close out | **DONE 2026-09-23** | ledger activation note and RESULT for 14.6/14.7; process doc updated (PR #72). On the owner's say-so: dropped `valo_v4_rehearsal` (483 MB), `valo_v4_rehearsal2` (498 MB) and rc3's leftover `valo_rc3_rehearsal` (324 MB), each with no open connections, leaving `postgres`, `valo_rc3_test` (still `.env.test`'s write-test database, deliberately kept) and production. Removed the `main-worktree` (`6f45476`) and `release-tools` (`bcb83b9`) worktrees, both clean and on `main`. Still retained: `impact_scores_v3` through the recovery window, `impact_scores_v1` to 2026-10-03, and the `../vwft-v4-reference` worktree |
